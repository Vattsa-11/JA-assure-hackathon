...
# Target languages for localization. The source content is English ("en"),
# so "en" is deliberately absent — localizing en -> en makes no sense.
SUPPORTED_LANGUAGES = {
...
def resolve_target_languages(requested: list[str] | None) -> list[str]:
    """
    Normalize a requested language list. None/empty means "all supported
    languages". Unknown codes are dropped with a warning rather than raising,
    so a typo can't abort a whole pipeline run.
    """
    if not requested:
        return list(SUPPORTED_LANGUAGES.keys())
    unknown = [lang for lang in requested if lang not in SUPPORTED_LANGUAGES]
    if unknown:
        logger.warning(f"Ignoring unsupported target languages: {unknown}")
    return [lang for lang in requested if lang in SUPPORTED_LANGUAGES]


def localize_content(db: Session, asset_id: int, target_language: str) -> ContentAsset:
    """
    Localizes a piece of content with cultural adaptation (not word-for-word
    translation). Creates a new ContentAsset linked via source_asset_id, then
    runs the localized version through the Compliance Agent independently —
    a claim can become non-compliant only after translation.
    """
    if target_language not in SUPPORTED_LANGUAGES:
        raise ValueError(
            f"Unsupported language: {target_language}. Supported: {list(SUPPORTED_LANGUAGES.keys())}"
        )
...
"""
    ...
    # Independent compliance check on the localized version (master plan 3E).
    ...
    # check_compliance moves passed assets to pending_review; failed stay draft.
