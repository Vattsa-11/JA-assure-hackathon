...
class LLMClient:
    ...
    def _get_model(self, model_name: str, temperature: float = 0.7, json_mode: bool = False) -> Runnable:
        model = ChatGroq(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    raise
        raise RuntimeError("LLM text generation failed after retries")  # pragma: no cover
    ...
    def generate_json(self, prompt: str, system_prompt: str = "You are a helpful assistant. Always return valid JSON.", model_name: str = "llama3-8b-8192", temperature: float = 0.1) -> dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Failed to parse JSON from LLM response. Error: {e!s}. Response content: {response.content}")
                ...
                if attempt < max_retries - 1:
                    ...
                    raise
        raise RuntimeError("LLM JSON generation failed after retries")  # pragma: no cover
